...
class Interface:            # À compléter plus tard
    def __init__(self, largeur = 800, hauteur = 450):
        ...
        self.fecran_menu = pygame.image.load("images/Cartes_fond_ecran.jfif")
        self.fecran_menu = pygame.transform.scale(self.fecran_menu,(largeur, hauteur))

        self.bouton_play = pygame.image.load("images/Boutton_play.png")
        self.bouton_play = Bouton(largeur/2.4,hauteur/2.4,self.bouton_play)
    ...
    def loop(self):
        ...
        while self.alive:
            ...
            if self.menu == True:                     #ecran du menu
                self.frame.blit(self.fecran_menu,(0,0))
                for event in pygame.event.get():  ############ BOUCLE DES ÉVÈNEMENTS
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and self.bouton_play.rect.collidepoint((self.souris.rect.x, self.souris.rect.y)):
                        self.menu = False
                ...
                self.bouton_play.update(self.frame)
            ...
            if self.menu == False:                 # Ecran du Jeux
                ...
                self.updatePlateau()
                self.bouton_quitter.update(self.frame)                  # update le plateau (affiche les cartes, gère la carte en main, ect...)
                if self.test_musique == True:
                    self.bouton_musiqueOn.update(self.frame)
                if self.test_musique == False:
                    self.bouton_stopmusique.update(self.frame)
